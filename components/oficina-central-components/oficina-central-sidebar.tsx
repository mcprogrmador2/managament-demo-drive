    ...
    navRevision: [
      {
        ...
        onClick: () => router.push("/oficina-central/proyectos-cerrados"),
        icon: FolderCheck,
        path: "/oficina-central/proyectos-cerrados"
      },
      {
    ...
    navDocumentoFinal: [
      {
        ...
        onClick: () => router.push("/oficina-central/documentos-finales"),
        icon: FileText,
        path: "/oficina-central/documentos-finales"
      },
    ],
    navControl: [
      {
        title: "Revisión",
        onClick: () => router.push("/oficina-central/revision"),
        icon: CheckCircle2,
        path: "/oficina-central/revision"
      },
    ],
    navConfig: [
      {
      ...
      {
        title: "Configuración",
        onClick: () => router.push("/oficina-central/configuracion"),
        icon: Settings,
        path: "/oficina-central/configuracion"
      },
    ],
  };
  ...
  return (
    <aside className={`${isCollapsed ? 'w-16' : 'w-72'} h-screen bg-sidebar border-r border-sidebar-border flex flex-col transition-all duration-300`}>
      <div className={`${isCollapsed ? 'p-2' : 'p-4'} border-b border-sidebar-border`}>
        ...
        >
          <CheckCircle2 className={`${isCollapsed ? 'w-8 h-8' : 'w-12 h-12'} text-primary`} />
          {!isCollapsed && (
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className={`${isCollapsed ? 'p-2' : 'p-4'}`}>
          ...
          <div className="space-y-1">
            {data.navDocumentoFinal.map((item) => {
              const isActive = pathname === item.path;
              return (
                <button
                  key={item.title}
